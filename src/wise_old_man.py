...
class WiseOldManClient:
    ...
    async def get_top_placements_players(self, players: List[wom.GroupHiscoresEntry], placements:int = 1) -> Tuple[List[wom.Player], List[wom.Player]]:
        ...
        top_placements_iron_players = [player for player in players if player.player.type in (wom.PlayerType.Ironman, wom.PlayerType.Hardcore, wom.PlayerType.Ultimate)][:placements]
        return  top_placements_normal_players, top_placements_iron_players

    async def get_top_placements_hiscores(self, metric: wom.Metric, group_id: int = KITTY_GROUP_ID, number_of_ranks: int = 30, placements: int = 1)-> Tuple[List[wom.Player], List[wom.Player]]:
